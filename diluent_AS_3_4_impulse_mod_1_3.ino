...
#define MOTOR_COAST     218
#define MOTOR_SEALTURN1 272
...
#define PULSES                  9 //numer of times the hotwire pulses
#define PULSE_ON_TIME           250 //length of time the hotwire is on per pulse
#define PULSE_OFF_TIME          250 //length of time the hotwire is off per pulse
#define PULSE_WARMUP_TIME       300
...
void setup()
{
  ...
  // Initialize sealer
  {
    ...
    pwm.setPWM(STATION_SEAL_MOTOR, 0, 300);
...
//handle_pause:
  ...
  if (global_state == STATIONS) {
    ...
    // Seal station
   pwm.setPWM(STATION_SEAL_MOTOR, 0, MOTOR_SEALTURN1);
   seal_state = SEAL_TURN_1;
    ...
    while ((cut_state != CUT_OFF || fill_state != FILL_OFF || seal_state != SEAL_OFF)) {
      ...
      if(seal_state == SEAL_CLOSE){
        pwm.setPWM(STATION_SEAL_MOTOR, 0, MOTOR_COAST);//multi send what ever
        if(impulse_state == pulse_OFF && millis() >= pulseStart){
          pulseEnd = millis() + PULSE_WARMUP_TIME;
          pwm.setPWM(STATION_SEAL_HOTWIRE, 0, HOTWIRE_ON);
          impulse_state = pulse_ON;
         }
        else if(impulse_state == pulse_ON && millis() >= pulseEnd){
          pwm.setPWM(STATION_SEAL_HOTWIRE, 0, HOTWIRE_OFF);
          impulse_state = pulse_OFF;
          pulseStart = millis() + PULSE_OFF_TIME;
          seal_state = SEAL_IMPULSE;
          //pwm.setPWM(STATION_SEAL_MOTOR, 0, MOTOR_0); //moved down exparamentaly
          }
      }
      if(seal_state == SEAL_IMPULSE){
        if(impulse_state == pulse_OFF && pulse < PULSES && millis() >= pulseStart){
          pulseEnd = millis() + PULSE_ON_TIME;
          pwm.setPWM(STATION_SEAL_HOTWIRE, 0, HOTWIRE_ON);
          impulse_state = pulse_ON;
          pulse ++;
          pwm.setPWM(STATION_SEAL_MOTOR, 0, MOTOR_0);//moved here
          }
        else if(impulse_state == pulse_ON && millis() >= pulseEnd){
          pwm.setPWM(STATION_SEAL_HOTWIRE, 0, HOTWIRE_OFF);
          impulse_state = pulse_OFF;
      ...
      Serial.println(analogRead(BELT_WAIT));
      //delay(1);
    }
